...
export default function InstructorList() {
  ...
  const loadInstructors = useCallback(async () => {
    try {
      ...
      const response = await apiFetch(`/api/instructors/search?${params.toString()}`);
            ...
            <tbody>
              {backendData.map((ins) => (
                <tr
                  ...
                  onClick={isAdmin ? () => navigate(`/instructors/${ins.instructor_id}`) : undefined}
                  title={isAdmin ? `Click to navigate to Edit form for Instructor ${ins.instructor_code}` : 'Only admins can edit records'}
                >
